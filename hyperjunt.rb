...
# ----------------------------------- PostgreSQL
inject_into_file '.gitignore', after: "\0" do
  <<-CODE
  config/database.yml
  CODE
end
db = ask 'Database name (will be suffixed by environment):'
user = ask 'Database username:'
password = ask 'Database password:', echo: false
file 'config/database.yml.example', <<-CODE, force: true
default: &default
  adapter: postgresql
  host:
  username: #{user}
